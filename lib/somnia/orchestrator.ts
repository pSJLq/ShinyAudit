...
function renderSynthSystem(intent: ReturnType<typeof detectIntent>): string {
  ...
  return [
    ...
    "- BEFORE writing the verdict, scan the ENTIRE tool history for any non-empty values from: identity_best, identity_opensea_handle, identity_ens_resolved, identity_farcaster_handle, identity_lens_handle, identity_mirror_handle, identity_galxe_handle, address_ens_domain, address_public_name, identity_opensea, identity_farcaster, identity_ens_app.",
    "- For EACH non-empty hit, include a line in the verdict like: \"`0xABC…` is **`<handle>`** on <source>\".",
    "- NEVER conclude 'no identity link found' when even ONE of those tools returned a non-empty value.",
    "- If multiple sources agree (OpenSea + Farcaster both say 'foo'), lead the verdict with that confirmed handle.",
    "- If identity_best returned a value, that's the strongest signal — use it in the headline Verdict.",
    "",
