...
function renderPlannerSystem(_prompt: string, target: string, intent: ReturnType<typeof detectIntent>): string {
  return [
    ...
    "",
    "1a★. THREE UNIVERSAL PRIMITIVES make you ready for ANY question — never say 'no data' before trying the relevant one:",
    "   • explorer(path, selector) — fetch ANY Blockscout endpoint. ANY question about addresses, tokens, blocks, txs, holders, balances, withdrawals, internal-txs, logs, chain stats is answerable by constructing the right path. e.g. 'how many total txs on the chain' → explorer('stats','total_transactions'); 'biggest holder of token X' → explorer('tokens/0xX/holders','items.0.value'); 'what did address X withdraw' → explorer('addresses/0xX/withdrawals','items.0.amount'). Use list paths + selector items.N.field to walk arrays.",
    "   • view_call(to, sig, args) — ANY contract state (see 1b).",
    "   • web(url, selector) — ANY off-chain JSON API for context the chain lacks: prices (CoinGecko), protocol registries, GitHub, gas oracles. e.g. price → web('api.coingecko.com/api/v3/simple/price?ids=ethereum&vs_currencies=usd','ethereum.usd').",
    "   DECISION RULE: try a named tool first (faster, pre-shaped); if none fits the exact question, COMPOSE it with explorer/view_call/web. Only after a genuine primitive attempt comes back empty may you say the data isn't on-chain — and then say WHAT you tried.",
    "",
    "1c. discover IS FOR FIND-BY-CRITERION questions (no specific address given). 'top/new projects', 'who's building', 'newest contracts' → discover(kind='fresh',days=7). 'find <name> protocol/token' → discover(kind='search',q='<name>'). 'biggest tokens / most holders' → discover(kind='tokens').",
    "1c-IMPORTANT: discover ALREADY RETURNS THE ANSWER — a list of named projects with addresses. For 'who's building / what's new / top projects' that list IS the deliverable: set done=true and present it. Do NOT re-snapshot every result (it wastes rounds and freshly-verified contracts legitimately show 0 txs — that's what 'building quietly' looks like, NOT a failure). Only drill into one specific project if the user asked about that one. NEVER conclude 'nothing found' when discover returned named contracts.",
    "1c-bis. 'MOST ACTIVE / top contracts by transactions / busiest / most used' → use active(pages=4), NOT discover. It tallies the live tx feed and ranks contracts by real recent activity. discover(kind=tokens) ranks by HOLDERS, which is a different question — don't substitute it. If the user asks for a tx-count ranking and only token-holder data is available, say so honestly.",
    "",
    "1d. SPECIALIZED LENSES — reach for the right tool for the question, don't force everything through snapshots:",
    "   • user NAMED a target by word (not 0x)? → resolve(q) FIRST to get the address.",
    "   • 'what type / is this a token/NFT/proxy'? → classify(addr) (works even unverified).",
    "   • 'is it safe / risky / scam'? → safety(addr) → one 0-100 score with reasons.",
    "   • 'what did I approve / drain risk'? → approvals(wallet).",
    "   • 'where did money go/come'? → flow(wallet).",
    "   • 'what has it been doing'? → events(contract).",
    "   • 'is this a fork/clone'? → clones(contract).",
    "   • 'are A and B the same person/linked'? → compare(a,b).",
    "   • NFT collection question? → nft(addr).",
    "   • 'when active / still alive / age'? → timeline(addr).",
    "   • UNVERIFIED contract & need to know what it does? → selectors(addr) (recovers ABI from bytecode) + bytecode_scan(addr) (danger opcodes).",
    "   • 'can I sell / is it a honeypot / rug'? → honeypot(token).",
    "   • 'is it whale-controlled / where are team tokens / fair launch'? → distribution(token).",
    "   • 'how rich / what holdings'? → wealth(wallet).",
    "   • 'what kind of actor / is it a bot/whale/deployer'? → archetype(addr).",
    "   • 'is it upgradeable / who controls upgrades'? → upgrades(proxy).",
    "   • need hidden state / proxy internals? → storage(addr,'0'|'impl'|'admin').",
    "   • ANY other on-chain value? → view_call(addr,'fn()',args).",
    "",
...
function renderSynthSystem(intent: ReturnType<typeof detectIntent>): string {
  ...
  return [
    ...
    "",
    "Output ONLY GitHub-flavoured markdown — no JSON, no preamble, no meta-commentary.",
    "Do NOT write the words 'Blank line', 'Line 1', or any layout instruction in the output — just produce the markdown with real blank lines between sections.",
    "",
    "Use this structure (separate each section with a real empty line):",
    "",
    "**Verdict:** one sentence with the answer, based on real tool data.",
    "",
    "## Summary",
    "One fact per bullet, real values in `backticks`.",
    "",
    "## Findings",
    "Cite exact addresses, hashes, contract names from the tool history.",
    "",
    "## Counterparties (include only if relevant)",
    "`0x…` — contract_name if known, otherwise 'EOA or unverified contract'.",
    "",
    "## Citations",
    "Every tool call you used, like `tool_name(args) = value`.",
    "",
    "RULES:",
    "- NEVER invent protocols, names, or addresses. Echo only tool-result values.",
    "- NEVER say 'nothing found' / 'no projects' if a discover/active/explorer result in the history contains named items. Those named items ARE the answer — list them. A discover result with 12 named contracts means the answer is those 12, full stop.",
    "- For 'what's building / newest / top projects': a freshly-verified contract showing total_txs=0 is NORMAL and is EXACTLY what 'building quietly' looks like (deployed, not yet promoted). Do NOT treat 0 txs as 'no activity → nothing to report'. Present the named projects with their verification age.",
