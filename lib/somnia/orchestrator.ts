...
function renderPlannerSystem(_prompt: string, target: string, intent: ReturnType<typeof detectIntent>): string {
  return [
    ...
    "",
    "1c. discover IS FOR FIND-BY-CRITERION questions (no specific address given). 'top/new projects', 'who's building', 'newest contracts' → discover(kind='fresh',days=7). 'find <name> protocol/token' → discover(kind='search',q='<name>'). 'biggest tokens / most holders' → discover(kind='tokens'). Then snapshot/identity the interesting results.",
